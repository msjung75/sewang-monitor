...
export async function collectPermits(query, key, {fetcher=fetch}={}) {
  ...
  const jobs = [];
  for (const t of types) for (const p of prefixes) jobs.push(fetchService(t, p, since, until, key, maxPages, dateField, datefmt, colOverride, debug, fetcher));
...
async function fetchService(typeKey, addrPrefix, since, until, key, maxPages, dateField, datefmt, colOverride, debug, fetcher) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    let data;
    try {
      const r = await fetcher(url, { headers: { Accept: 'application/json' }, signal:AbortSignal.timeout(25000) });
      if(!r.ok)throw new Error('upstream_http_failure');
      const text = await r.text();
      data = JSON.parse(text);
    } catch (e) { throw new Error('upstream_request_failed'); }
