...
import { privateAccessConfigured, readPrivateAccess, writePrivateAccess } from './access-store.mjs';
...
export async function allowlist(options) {
  let data = null;
  if (privateAccessConfigured(options)) data = await readPrivateAccess(options);
  if (!data) {
    data = await repoJson('data/allowlist.json', options);
    if (privateAccessConfigured(options)) await writePrivateAccess(data, options);
  }
  if (!data || (!Array.isArray(data) && !Array.isArray(data.users))) throw new Error('invalid_access_configuration');
  return { users: Array.isArray(data) ? data : data.users, blocked: data.blocked || [] };
}
